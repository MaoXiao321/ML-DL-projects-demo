...
目前电商平台的可参考淘宝的API接口，地址为:https://suggest.taobao.com/sug?area=etao&code=utf-8&callback=KISSY.Suggest.callback&q=需要扩写的query
百度的：http://suggestion.baidu.com/su?wd=需要扩写的query&p=3&cb=window.bdsug.sug      或是https://www.baidu.com/sugrec?pre=1&p=3&ie=utf-8&json=1&prod=pc&from=pc_web&wd=需要扩写的query
搜狗的：http://w.sugg.sogou.com/sugg/ajaj_json.jsp?key=需要扩写的query&type=web
必应的：http://sg1.api.bing.com/qsonhs.aspx?type=cb&cb=callback&q=需要扩写的query
...
def get_similarity(sentences):
    ...
    # 得到4个句子的向量表示，此处是平均所有位置的输出值mean_pooled
